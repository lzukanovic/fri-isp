...
iptables -A OUTPUT -p tcp --dport 80 -j ACCEPT # HTTP request
iptables -A INPUT  -p tcp ! --syn --sport 80 -j ACCEPT # HTTP response
...
################
### HTTPS settings
### (6) Allow outgoing HTTPS connections
iptables -A OUTPUT -p tcp --dport 443 -j ACCEPT
iptables -A INPUT -p tcp ! --syn --sport 443 -j ACCEPT

### (7) Allow incoming HTTPS connections
...
#################
### ICMP settings
### (8) Allow outgoing ping requests (and corresponding ping replies)
### Hint: use protocol icmp and set the type of the message to either request
### or reply.
iptables -A OUTPUT -p icmp --icmp-type echo-request -j ACCEPT
iptables -A INPUT -p icmp --icmp-type echo-reply -j ACCEPT

### (9) Allow incoming pings but only from a specific IP address (you may decide which one)
